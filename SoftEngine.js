    ...
    Device.prototype.render = function (camera, meshes, axis) {
      ...
      for (var index = 0; index < meshes.length; index++) {
        ...
        //  var kForColor = (indexFaces / 6) >> 0;
          ...
          // var colorRGBA;
          // colorRGBA = new HELPER.Color4(Math.random(), Math.random(), Math.random(), 1);
          ...
          colorRGBA = new HELPER.Color4(0, 0, 0, 1);
          if (currentFace.NA != 1 && currentFace.NA2 != 1)
          	this.drawBline(pixelA, pixelB, colorRGBA);
          if (currentFace.NA != 2 && currentFace.NA2 != 2)
          	this.drawBline(pixelB, pixelC, colorRGBA);
          if (currentFace.D != -1) {
          	if (currentFace.NA != 3 && currentFace.NA2 != 3)
            	this.drawBline(pixelC, pixelD, colorRGBA);
            if (currentFace.NA != 4 && currentFace.NA2 != 4)
            	this.drawBline(pixelD, pixelA, colorRGBA);
          } else {
          	if (currentFace.NA != 4 && currentFace.NA2 != 4)
            	this.drawBline(pixelC, pixelA, colorRGBA);
          }
        }
